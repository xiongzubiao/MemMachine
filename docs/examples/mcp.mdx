...
This will start MemMachine (and Neo4j if configured) in Docker containers. The MemMachine container will be named `memmachine-app`.
...
7. Save the configuration file and restart the Claude Desktop App.  Note that the logs are written to `stderr` as well
   so they will be available in Claude's log folder.  Check that log if anything went wrong.
...
```
  ...
  Here is the list of supported headers you can pass to the MCP server:
